...
Sebelum kita menerapkan metode machine learning, penting untuk membagi dataset jadi minimal dua kelompok:

1. Data pelatihan (train set) 60-75% 

2. Data uji (test set) 25-40%.

Pembagian data tersebut disebut sebagai cross validation. Data test/data uji merupakan data yang digunakan untuk melihat bagaimana kinerja model pada data baru atau digunakan sebagai data yang akan diprediksi, di mana nantinya kita akan membandingkan hasil prediksi dengan nilai sesungguhnya, kita akan mensample secara random index pada data marketing. 75% data akan dijadikan sebagai data train, dan 25% data akan dijadikan sebagai data test. 
```{r}
set.seed(123)
...
head(test,3)
```
Setelah data dibagi menjadi data train dan data test, selanjutnya membuat pemodelan machine learning.Dari studi kasus diatas, target variabelnya adalah Sales, Untuk memprediksi target variabel Sales tersebut, kita menggunakan satu prediktor, yaitu variabel Facebook. Untuk membuat model regresi linear sederhana pada R
```{r}
